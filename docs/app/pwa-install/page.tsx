    ...
    <article className="rounded-[18px] border border-line bg-surface p-6 shadow-[0_20px_60px_rgba(0,0,0,0.10)] sm:p-8">
      <header className="mb-8">
        ...
        <p className="text-sm leading-6 text-muted">
          ...
          &ldquo;Install app&rdquo; button instead of relying on the browser&apos;s
          default banner. It tracks when the browser is ready to install, whether the app
          is already installed, and triggers the native prompt on demand. On React Native
          and during server rendering it is a safe no-op, so the same button simply stays
          hidden.
        </p>
      </header>

      <section className="border-t border-line py-6">
        <h2 className="mb-3 text-xl font-bold text-fg">What the hook returns</h2>
        <div className="overflow-x-auto">
          <table className="w-full min-w-[520px] border-collapse text-sm">
            <thead>
              <tr className="text-left text-fg">
                <th className="border-b border-line py-2 pr-4 font-semibold">Field</th>
                <th className="border-b border-line py-2 pr-4 font-semibold">Type</th>
                <th className="border-b border-line py-2 font-semibold">Description</th>
              </tr>
            </thead>
            <tbody className="text-muted">
              <tr>
                <td className="border-b border-line py-2 pr-4 align-top font-mono text-accent">
                  canInstall
                </td>
                <td className="border-b border-line py-2 pr-4 align-top font-mono">
                  boolean
                </td>
                <td className="border-b border-line py-2 align-top leading-6">
              ...
              <tr>
                <td className="py-2 pr-4 align-top font-mono text-accent">
                  promptInstall
                </td>
      ...
      <Step number={1} title="Make the app installable">
        <p>
          The browser only fires an install prompt when the app meets its install
          criteria: a web app manifest with an icon set and a registered service worker,
          served over HTTPS. Localhost counts as secure during development.
        </p>
        <p>
...
function InstallButton() {
  ...
  // Render your button here.
        ...
        <p>
          ...
          <code className="font-mono">false</code> until the browser is ready, so the
          button appears only when installing will actually work.
        </p>
      </Step>

      <Step number={3} title="Trigger the prompt from a user action">
        <p>
          Call <code className="font-mono">promptInstall()</code> directly from a click.
          The browser blocks the prompt if it is not tied to a user gesture.
        </p>
        <CodeBlock
          lang="tsx"
          code={`return (
  ...
  >
    Install app
      ...
      <Step number={4} title="Account for platforms without a prompt">
        <p>
          Some platforms — notably iOS Safari — do not expose a programmatic install
          prompt. There <code className="font-mono">canInstall</code> stays{' '}
          ...
          that case, no extra branching is required; add your own hint if you want to
          guide those users.
        </p>
      </Step>
    </article>
